...
namespace CoinPouch {
    public sealed unsafe class Addon {
        ...
        private static Dictionary<string, int> lastChildCount = new();
        ...
        public static bool HasAddonChildCountChanged(string name) {
            int currentChildCount;
            bool changed = false;

            if(IsAddonOpen(name)) {
                AtkUnitBase* addon = GetAddon(name);
                currentChildCount = addon->UldManager.NodeListCount;
            } else {
                currentChildCount = 0;
            }

            if(!lastChildCount.ContainsKey(name) || lastChildCount[name] != currentChildCount) {
                changed = true;
            }

            lastChildCount[name] = currentChildCount;
